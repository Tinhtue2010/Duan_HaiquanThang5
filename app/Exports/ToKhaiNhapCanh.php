...
class ToKhaiNhapCanh implements FromArray, WithEvents, WithDrawings
{
    protected $ma_nhap_canh;
    protected $data = [];

    public function __construct($ma_nhap_canh)
    {
        $this->ma_nhap_canh = $ma_nhap_canh;
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $nhapCanh = NhapCanh::find($this->ma_nhap_canh);
                $doanhNghiep = DoanhNghiep::find($nhapCanh->ma_doanh_nghiep);
                ...
                $sheet->setCellValue('C10', $nhapCanh->PTVTXuatCanh->ten_phuong_tien_vt);
                ...
                $sheet->setCellValue('C22', $nhapCanh->ten_thuyen_truong);
                ...
                $sheet->getStyle('A23:B26')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);

                $sheet->mergeCells('C23:D26');
                $sheet->setCellValue('C23',  $nhapCanh->PTVTXuatCanh->so_giay_chung_nhan);
                ...
                $sheet->mergeCells('A27:B28');
                $sheet->mergeCells('C27:D28');
                $sheet->setCellValue('C27', "10. Dung tích có ích:");
                $sheet->setCellValue('C28', "Net tonnage");

                $this->applyOuterBorder($sheet, 'A27:D28');
                
                $sheet->getStyle('A27:D28')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT)->setVertical(Alignment::VERTICAL_TOP);

                //29-30
                $sheet->mergeCells('A29:D29');
                $sheet->mergeCells('A30:D30');
                $sheet->setCellValue('A29', "11. Vị trí tàu tại cảng: Vùng neo Vạn Gia");
                $sheet->setCellValue('A30', "Position of the ship in the port (berth of station)");
                ...
                if($nhapCanh->is_khong_hang == 0){
                    ...
                    $sheet->setCellValue('A36', $nhapCanh->so_luong . " Kiện");
                    $sheet->setCellValue('B36', $nhapCanh->trong_luong . " Tấn");
                    $sheet->setCellValue('C36', $nhapCanh->ten_hang_hoa);

                } else {
                    $sheet->mergeCells('A36:J36');
                    $sheet->setCellValue('A36', "NIL");
                }
                ...
                $sheet->setCellValue('D39', "NIL");
                $sheet->setCellValue('F39', "NIL");
                $sheet->setCellValue('H39', "NIL");

                $sheet->setCellValue('A40', "Nhập khẩu(Import cargo)");
                $sheet->setCellValue('D40', $nhapCanh->is_khong_hang == 0 ? $nhapCanh->loai_hang : 'NIL');
                if($nhapCanh->is_khong_hang == 0){
                    $sheet->setCellValue('F40', "(" . $nhapCanh->so_luong . " Kiện)");
                } else {
                    $sheet->setCellValue('F40', "NIL");
                }
                if($nhapCanh->is_khong_hang == 0){
                    $sheet->setCellValue('H40', $nhapCanh->don_vi_tinh. " TẤN");
                } else {
                    $sheet->setCellValue('H40', "NIL");
                }
                $sheet->setCellValue('H40', $nhapCanh->is_khong_hang == 0 ? "KIỆN/TẤN" : 'NIL');
                ...
                $A54 = $this->createRichText("17. Bản khai hàng hóa:  ", "NIL");
                $D55 = $this->createRichText("Ship's Stores Declaration ", "01");
                ...
                //56-58
                $A57 = $this->createRichText("Crew List ", "01");
                ...
                $A60 = $this->createRichText("Crew's Effects Declaration", "(*)");
                $D59 = $this->createRichText("23. Bản khai kiểm dịch y tế", "(*)");
                $D61 = $this->createRichText("Heath", "(*)");
                ...
                $boldText = $richText->createTextRun($nhapCanh->PTVTXuatCanh->draft_den ?? '');
                $boldText->getFont()->setBold(true)->setName('Times New Roman');
                $boldText->getFont()->setSize(10);

                $richText->createText("\n");
                $boldText = $richText->createTextRun($nhapCanh->PTVTXuatCanh->loa_den ?? '');
                $boldText->getFont()->setBold(true)->setName('Times New Roman');
                $boldText->getFont()->setSize(10);

                $richText->createText("\n");
                $boldText = $richText->createTextRun($nhapCanh->PTVTXuatCanh->breadth_den ?? '');
                $boldText->getFont()->setBold(true)->setName('Times New Roman');
                $boldText->getFont()->setSize(10);
                ...
                //56-58
                $sheet->mergeCells("F56:J56");
                $sheet->mergeCells("F57:J58");
                $sheet->setCellValue('F56', "CHỦ HÀNG: ". $nhapCanh->doanhNghiepChon->ten_doanh_nghiep ?? '');
                $sheet->setCellValue('F57', "ĐỊA CHỈ: ". $nhapCanh->doanhNghiepChon->dia_chi ?? '');
                ...
                $sheet->setCellValue('F72', $nhapCanh->ten_thuyen_truong);
    ...
    public function drawings()
    {
        $drawings = [];
        $nhapCanh = NhapCanh::find($this->ma_nhap_canh);
        if (in_array($nhapCanh->trang_thai, ["2"])) {
            $qrCodeText = 'Số tờ khai nhập cảnh: ' . $nhapCanh->ma_nhap_canh .
