...
@section('title', 'Quản lý nhập cảnh')
        ...
        <div class=" px-4">
            <div class="card shadow mb-4">
                <div class="card-header pt-3">
                    ...
                    <div class="row">
                        <div class="col-6">
                            <h4 class="font-weight-bold text-primary">Danh sách tờ khai nhập cảnh</h4>
                        </div>
                        <div class="col-6">
                            @if (Auth::user()->loai_tai_khoan == 'Doanh nghiệp')
                                <a href="/them-to-khai-nhap-canh">
                                    <button class="btn btn-success float-end">Nhập tờ khai
                                        nhập cảnh</button>
                                    </a>
                            @endif
                        </div>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#dataTable').DataTable({
                ...
                serverSide: true,
                stateSave: true,
                ajax: "{{ route('nhap-canh.getNhapCanhs') }}",
                ...
                createdRow: function(row, data, dataIndex) {
                    $(row).addClass('clickable-row').attr('onclick',
                        `window.location='{{ url('/thong-tin-nhap-canh') }}/${data.ma_nhap_canh}'`
                    );
                },
                initComplete: function() {
                    ...
                    select.append(
                        '<option class="text-success" value="3">ĐÃ DUYỆT THỰC nhập</option>'
                    );
                    select.append(
