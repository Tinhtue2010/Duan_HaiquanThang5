...
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-6">
                    ...
                    @if (trim($yeuCau->trang_thai) != '0')
                        <a
                            href="{{ route('quan-ly-kho.in-phieu-chuyen-tau-cont', ['ma_yeu_cau' => $yeuCau->ma_yeu_cau]) }}">
                        ...
                        <tbody class="clickable-row">
                            @foreach ($chiTiets as $index => $chiTiet)
                                <tr class="text-center" data-chitiet='@json($chiTiet)'>
                                    ...
                                    <td>{{ $chiTiet->so_container_goc }}</br> ({{ $chiTiet->tau_goc }})</td>
                                    <td>{{ $chiTiet->so_luong_chuyen }}</td>
                                    <td>{{ $chiTiet->so_container_dich }}</br> ({{ $chiTiet->tau_dich }})</td>
                                    <td>
                                    ...
                                    @if ($yeuCau->trang_thai == '2')
                                        <td>
                                            <a
                                                href="{{ route('export.theo-doi-tru-lui', ['cong_viec' => 2, 'ma_yeu_cau' => $yeuCau->ma_yeu_cau, 'so_to_khai_nhap' => $chiTiet->so_to_khai_nhap]) }}">
                                                <center>
                            ...
                            @elseif(trim($yeuCau->trang_thai) == '3')
                                ...
                                <div class="row">
                                    <center>
                                        <div class="col-6">
                                            <a
                                                href="{{ route('quan-ly-kho.xem-sua-yeu-cau-tau-cont', ['ma_yeu_cau' => $yeuCau->ma_yeu_cau]) }}">
                                                <button class="btn btn-warning px-4">
                                                    <img class="side-bar-icon"
                                                        src="{{ asset('images/icons/edit.png') }}">
                                                    Xem sửa đổi
    ...
    <div class="modal fade" id="xacNhanModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('quan-ly-kho.duyet-yeu-cau-tau-cont') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <h5>Xác nhận duyệt yêu cầu chuyển container và tàu ?</h5>
    ...
    <div class="modal fade" id="xacNhanHuyModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('quan-ly-kho.huy-yeu-cau-tau-cont') }}" method="POST">
                    @csrf
    ...
    <div class="modal fade" id="thayDoiCongChucModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('quan-ly-kho.thay-doi-cong-chuc-tau-cont') }}" method="POST">
                    <div class="modal-body">
    ...
    <script>
        ...
        $(document).on('click', '.xemChiTiet', function() {
            ...
            chiTiet.yeu_cau_tau_cont_hang_hoa.forEach((hangHoa, index) => {
                let row = `<tr>
                       <td>${index + 1}</td>
