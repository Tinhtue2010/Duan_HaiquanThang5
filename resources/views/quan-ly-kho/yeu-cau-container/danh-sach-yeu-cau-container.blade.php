...
@section('title', 'Quản lý yêu cầu chuyển container')
        ...
        <div class=" px-4">
            <div class="card shadow mb-4">
                <div class="card-header pt-3">
                    ...
                    <div class="row">
                        <div class="col-9">
                            <h4 class="font-weight-bold text-primary">Danh sách yêu cầu chuyển container</h4>
                        </div>
                        <div class="col-3">
                            @if (Auth::user()->loai_tai_khoan == 'Doanh nghiệp')
                                <a href="{{ route('quan-ly-kho.them-yeu-cau-container') }}"><button
    ...
    <script>
        $(document).ready(function() {
            var table = $('#dataTable').DataTable({
                ...
                serverSide: true,
                stateSave: true,
                ajax: "{{ route('quan-ly-kho.getYeuCauContainer') }}",
                ...
                createdRow: function(row, data, dataIndex) {
                    $(row).addClass('clickable-row').attr('onclick',
                        `window.location='{{ url('/thong-tin-yeu-cau') }}/${data.ma_yeu_cau}'`
                    );
                },
                initComplete: function() {
                    ...
                    select.on('change', function() {
                        ...
                        localStorage.setItem('container', val);
                        column.search(val ? val : '', false, true).draw();
                    });

                    var savedFilter = localStorage.getItem('container');
                    if (!savedFilter) {
                        savedFilter = '';
                        localStorage.setItem('container', savedFilter);
                    }
